...
import { prepareLocalCliRuntimeConfig, ensureLocalCliOpenCodeModelConfiguredAndAvailable } from "./local-models.js";
...
function classifyLocalCliErrorCode(errorMessage: string): "UNAVAILABLE" | "MODEL_INVALID" | "EXECUTION_FAILED" {
  if (/configured opencode model is unavailable|returned no models|requires `adapterconfig\.model`|provider\/model format/i.test(errorMessage)) {
    return "MODEL_INVALID";
  }
  if (/not executable|command/i.test(errorMessage)) {
    return "UNAVAILABLE";
  }
...
export async function executeLocalCli(
  ...
  try {
    ...
    ): AdapterExecutionResult => parseOpencodeFullExecutionResult({
      ...(attempt.proc.timedOut
        ...
        : (() => {
            ...
            return {
              ...
              signal: attempt.proc.signal,
              timedOut: false,
              errorCode: (synthesizedExitCode ?? 0) === 0
                ? undefined
                : /unknown session|session\b.*\bnot found|stale/i.test(fallbackErrorMessage)
                  ? "SESSION_INVALID_OR_STALE"
                  : /not executable|command/i.test(fallbackErrorMessage)
                    ? "UNAVAILABLE"
                    : "EXECUTION_FAILED",
    ...
    return parseOpencodeFullExecutionResult({
      ...
      signal: null,
      timedOut: false,
      errorCode: classifyLocalCliErrorCode(errorMessage),
      errorMessage,
      sessionParams: null,
